+++
title = "Part 1: Type Inference"
date = "2023-05-01T16:56:18Z"
author = "thunderseethe"
tags = ["Programming Languages", "Type Inference"]
keywords = ["Programming Languages", "Type Inference", "Bidirectional Typechecking", "Unification", "Union-Find", "Constraint Solving"]
description = ""
+++

# Part 1: Type Inference
I'd like to write a programming language, more specifically a compiler for a programming language I'm going to make up. This is not the first time I've wanted to do that. In fact, I've had the [itch](https://github.com/thunderseethe/waht) [quite a](https://github.com/thunderseethe/panera) [few](https://github.com/thunderseethe/brainfuck_interpreter) [times](https://github.com/thunderseethe/false_interpreter) [before](https://github.com/thunderseethe/tiger). I can't tell you why I keep returning to this venture when I've failed at it so many times. What I can tell you is where I always fail. Each time I begin a sparkly new project with fresh eyes and quickly whip together a lexer.
 ...
 * Infographic for Lexing, Parsing, Semantic Analysis, Code Gen, etc.
...
We're focused on implementing type inference (For brevity, we won't cover definitions of types, polymorphism, or ASTs). I've cobbled together an idea of how to do type inference from reading a lot of sources and looking at some production compilers. I couldn't find a good resource that tied all the concepts together, so I decided to write one. 

We'll build our language in Rust. We might consider a GC'd language such as OCaml or Haskell to make it easy to work with trees. The reason I've not opted for a more traditional compiler language is twofold. I'm more familiar with Rust, and Rust's borrow checker allows for safe mutation. Let's talk about how our AST will be designed in Rust.
...
When we begin constraint solving, our union-find already has a single set for each type variable with itself as the representative. We can think of this as a type substitution where every variable substitutes itself. As we unify things we'll call union to merge these sets. Anytime we merge two sets all the type variables in that set now map to the representative. If we need to substitute a type variable during unification (spoiler we will), we can do that by calling find to look up its current representative.


#### Back to Unification
Now armed with the knowledge of how to maintain our type substitution in near linear time, let's start looking at how we unify types. We'll start with a top level method `unification` that takes a set of constraints:
```rs
fn unification(
  &mut self, 
  constraints: Vec<Constraint>
...
```
Not a lot to see yet, for each constraint we call `unify_ty_ty` to check the types can unify and be made equal. If we encounter an error in any individual constraint we bail early and don't try to solve any more constraints.
...
```
When we normalize we'll look up the representative for our type variable in our union-find, and substitute it into our type. If we find `None` that means we map our variable to itself, so we just return it without substituting.

Great now that we've removed all the type variables we can with normalization we can break down our cases:
```rs
...
```
This case is where we actually solve type variables for their types. When a type variable encounters a (non-variable) type, we unify the two making our concrete type the representative of that type variable's set. Moving forward whenever we normalize that type variable, or a type variable in it's set, we'll return this type.

We also call `occurs_check` on our type before we unify, possibly returning an `InfiniteType` error. This check walks our type and makes sure it doesn't have any instance of the type variable we're about to unify it with. The reason we need to avoid this is it forms a cycle. When we go to normalize that type variable we'll look up it's type, and normalize it. But that type contains our variable. So we'll look up our type again and normalize it, looping infinitely! Infinite loops do not make for fast type inference, so before we can unify our variable and type we have to check we're not creating a cycle.

```rs
(left, right) => Err(TypeError::TypeNotEqual(left, right)),
```
Our final case covers all the combinations where our types don't line up (`(Int, Fun(...))`, etc.). This means our set of constraints has a contradiction, all we can do is return a type mismatch and bail early.    

With that we've successfully solved our set of constraints, producing a type substitution. That tidies up our constraint solving phase, all we have left is to tie it all together into a type inference engine.

## Tying the knot
We've generated and solved constraints, along the way we've built up 3 things:

   * An inferred type for our whole AST
   * A typed AST where all our variables are annotated with their types
   * A type substitution to map all our variables to their types

The final step in our inference is to use our type substitution to solve all our inferred types from constraint generation. We do this by walking our AST and normalizing each type we see using our type substitution. The code for this is pretty rote, so we won't cover it in detail, but if you want to see it check out the [full source]().

### Generalization
While we're walking our AST normalizing types we'll come across a case I haven't touched on yet. What do we do if our type substitution solves a type variable to itself (or another type variable)? Can such a thing even occur? It absolutely can, it's not even hard to construct such an example:
```rs
let x = Var(0);
